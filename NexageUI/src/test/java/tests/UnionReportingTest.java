...
public class UnionReportingTest extends BaseTest {
    ...
    public void createProjectTest() {
        String token = testSteps.Steps.getToken();
        ...
        TestModel[] testsListInJson = testSteps.Steps.getTestsListInJson();
        List<TestModel> testsFromPage = nexageProjectPage.getTestsFromPage();
        Steps.checkTestsListIsSorted(testsFromPage, testsFromPage.stream().sorted(Comparator.comparing(TestModel::getStartTime).reversed()).collect(java.util.stream.Collectors.toList()));
